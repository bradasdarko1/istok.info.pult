...
export default function StudyLevelPage({ level }: Props) {
  ...
  const renderUi = (value: string) => (language === 'sr' ? transformSr(value, script) : value)
  ...
  return (
    <main className="relative min-h-screen px-4 py-8 text-white sm:px-6 md:px-8 md:py-12">
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
        ...
        >
          <h1 className="mb-4 text-4xl font-extrabold tracking-[0.14em] uppercase sm:text-5xl">
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {filtered.map((uni, index) => (
            ...
            >
              ...
              <p className="mb-5 text-base leading-7 text-gray-300">{getText(uni.description)}</p>

              <div className="mb-4">
                <p className="mb-2 text-sm font-semibold tracking-wide text-white/70 uppercase">
              ...
              <p className="text-sm text-gray-400">
                <span className="font-semibold text-white/70">{renderUi(currentUi.note)}:</span>{' '}
                {getText(uni.note)}
              </p>
